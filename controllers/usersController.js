...
module.exports = {
    // Méthode pour récupérer les utilisateurs et les passer à la vue
    index: (req, res, next) => {
        User.find({})
        .then(users => {
            res.locals.users = users;
            res.locals.pageTitle = 'Liste des utilisateurs'; // Définir la variable pageTitle
            next();
        })
        .catch(error => {
    ...
    // Méthode pour afficher la vue de la liste des utilisateurs
    indexView: (req, res) => {
        res.render("users/index", { pageTitle: res.locals.pageTitle, users: res.locals.users }); // Passer pageTitle et users à la vue
    },
